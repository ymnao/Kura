...
1. [Releases](https://github.com/ymnao/Kura/releases) ページから最新版の `Kura.app.zip` をダウンロード
2. zip を解凍し、`Kura.app` を `/Applications/` に移動
3. ターミナルで quarantine 属性を剥がす（macOS の Gatekeeper 警告を回避するため）:

   ```bash
   xattr -dr com.apple.quarantine /Applications/Kura.app
   ```

4. `/Applications/Kura.app` を開く
5. 初回起動時にアクセシビリティ権限のプロンプトが出るので許可
