...
├── AppDelegate.swift                 NSStatusItem (本体 + セパレータ) / NSPopover の組み立て、折りたたみ、scan キャッシュ、ホットキー登録、環境設定変更購読
...
├── HotKeyManager.swift               Carbon RegisterEventHotKey でグローバルホットキーを登録、update() で再登録
├── HotKeyRecorderView.swift          環境設定でホットキーを録音するカスタム NSView (NSEvent.keyDown を capture)
├── AppOrderStore.swift               popover 内の並び順を UserDefaults に永続化（bundleIdentifier 配列）
├── AppExclusionStore.swift           蔵対象から除外する bundleId 集合を UserDefaults に永続化
├── PreferencesStore.swift            環境設定の永続化 (symbol / foldOnLaunch / launchAtLogin / hotKey)。SMAppService 統合、`KuraHotKey` 値型を export
├── PreferencesWindowController.swift 独立 NSWindow ベースの環境設定 UI (一般 / 対象アプリ / ホットキー タブ)
...
- **蔵本体** (`statusItem`): 通常 `variableLength`、常に右端側に表示。SF Symbol（展開中: 素のシンボル／折りたたみ中: `.fill` バリアント）で fold 状態を視覚化（`updateStatusIcon()` で差し替え。template image なのでメニューバー色に追従）。具体的なシンボルは `PreferencesStore.symbol` から読み出し（default `archivebox`、v0.7 環境設定 UI から `tray` / `shippingbox` / `square.stack` / `bubbles.and.sparkles`（カグラバチで蔵に物を出し入れする際の泡エフェクトモチーフ）に変更可能）
...
- 右クリック → メニュー（「折りたたむ／展開する」「環境設定…（⌘,）」「Kura を終了（⌘Q）」）
...
## グローバルホットキー（v0.5 で実装、v0.7 でカスタマイズ可能化）

折りたたみ／展開のトグルを既定 **⌃⌥⌘K** （Ctrl+Option+Cmd+K）で発火できる。
v0.7 環境設定の「ホットキー」タブから任意のキー組み合わせに変更可能（modifier 1 つ以上必須）。
...
- `init(keyCode:modifiers:handler:)` でホットキーを登録。アプリ寿命と同じライフタイムなので、`update` 経由の差し替えを除き `UnregisterEventHotKey` は呼ばず、プロセス終了時に OS が自動解除する（Swift 6 では `@MainActor` class の nonisolated deinit から non-Sendable な `EventHotKeyRef` を触れないため、deinit 自体を持たない）
- **`update(keyCode:modifiers:)`** で再登録経路を提供（v0.7 で追加）。登録中の `hotKeyRef` を instance プロパティに保持し、`update` 時に旧 ref を `UnregisterEventHotKey` してから新 ref を登録する。**差分判定 (`currentKeyCode` / `currentModifiers` との比較) は HotKeyManager 内部で行うため、同値での再呼び出しは no-op**。これにより呼び出し側 (`AppDelegate.handlePreferencesDidChange`) は無関係な設定変更通知でも安全に `update` を叩けて、再登録ロジックの状態が AppDelegate と HotKeyManager で二重管理になる事態を避けられる。旧 ref の解除が失敗しても新規登録は試みる（Carbon は非排他登録なので残った旧キーの実害は限定的）。handler は init 時のものを継続使用する（Kura は toggleFold だけが対象）
- 単一ホットキー前提（Kura は折りたたみトグルのみ使用）。複数ホットキーが必要になった時点で registry / ID 払い出しを足すのは trivial なので、現状は最小構成にしている
...
`AppDelegate.handlePreferencesDidChange` は symbol / foldOnLaunch / appExclusion / hotKey すべての変更で同じ `.kuraPreferencesDidChange` を受け取るが、AppDelegate 側では差分判定を持たず単に `hotKeyManager?.update(...)` を呼ぶだけ。差分判定は `HotKeyManager.update` 内部の `currentKeyCode`/`currentModifiers` 比較で行う（同値時は早期 return で `UnregisterEventHotKey` / `RegisterEventHotKey` を叩かない）。「再登録の状態管理」を HotKeyManager に閉じ込めて、AppDelegate が hotKey の前回値を覚えておく必要をなくす設計。
...
蔵アイコンのシンボル選択、起動時 fold 状態、Mac 起動時自動起動、対象アプリ除外リスト、ホットキーカスタマイズの 5 設定をユーザーが UI から変更できる。

### 構成

- `PreferencesStore.swift`: 値ストア。`UserDefaults` と `SMAppService` を 1 箇所に集約。値変更時に `.kuraPreferencesDidChange` を post し、`AppDelegate` が購読してアイコン再描画／ホットキー再登録する。ホットキー設定は `KuraHotKey` (`keyCode` / `modifiers` / `display`) 値型でまとめて保持
- `AppExclusionStore.swift`: 除外 bundleId 集合の永続化を担う薄いラッパ。`AppOrderStore` と同じく static methods 集約。`setExcluded(_:excluded:)` で `.kuraPreferencesDidChange` を post して popover を即時更新
- `PreferencesWindowController.swift`: 独立 NSWindow ベースの UI（`NSTabView` レイアウト、`isReleasedWhenClosed = false` でインスタンス保持）。AppDelegate が 1 個だけ生成して以降は再 show のみ
  - 「一般」タブ: symbol / 起動時 fold / 自動起動の 3 設定（NSStackView レイアウト）
  - 「対象アプリ」タブ: 蔵対象アプリ一覧 (NSTableView)。1 行 = チェックボックス + アイコン + アプリ名。チェックの ON/OFF が「蔵に入れる/入れない」と直感的に一致
  - 「ホットキー」タブ: 折りたたみ／展開トグルのキー組み合わせ録音 UI (`HotKeyRecorderView`) + 「デフォルトに戻す (⌃⌥⌘K)」ボタン
- `HotKeyRecorderView.swift`: System Settings 流のキー録音 UI。`NSView` サブクラスで、マウスクリック経由で focus を取った場合のみ recording モードに突入し、次の `keyDown` を capture して `KuraHotKey` を組み立てる (Tab navigation 経由は focus ring のみで recording に突入しない)。`NSEvent.ModifierFlags` → Carbon `cmdKey | optionKey | controlKey | shiftKey` への変換と、`kVK_*` ベースの特殊キー名 (↩ / ⇥ / Space / 矢印 / F1-F19 等) の表示文字列組み立てを担う
- 入り口: 蔵の右クリックメニュー「環境設定…」。`AppDelegate.showPreferences(_:)` で `setScanResult(lastScanResult)` を呼んで「対象アプリ」タブの行データを注入してから `showWindow(nil)` + `NSApp.activate(ignoringOtherApps: true)`

### 設定項目と挙動

| 設定 | 永続化 | 反映タイミング |
|---|---|---|
| `symbol` (蔵アイコン) | `UserDefaults "kura.symbol.name"` | 設定変更通知 → `rebuildIconCache()` → `updateStatusIcon()` |
| `foldOnLaunch` | `UserDefaults "kura.foldOnLaunch"` | `applicationDidFinishLaunching` で `pendingFoldOnLaunch` に積み、初回 warmup scan 成功時に消化 |
| `launchAtLogin` | `SMAppService.mainApp` 登録状態（UserDefaults には保存しない） | `SMAppService.mainApp.register / unregister` を直接呼ぶ |
| `appExclusion` (除外 bundleId 集合) | `UserDefaults "kura.appExclusion"` (`[String]` を sorted で保存) | 設定変更通知 → `handlePreferencesDidChange` → popover が開いていれば `visibleApps` で再フィルタ |
| `hotKey` (折りたたみトグル) | `UserDefaults "kura.hotKey.keyCode" / ".modifiers" / ".display"` (Int + Int + String) | 設定変更通知 → `handlePreferencesDidChange` → `HotKeyManager.update(keyCode:modifiers:)` (差分判定は HotKeyManager 内部) |

### 設計判断

- **NSWindow を独立で持つ理由**: popover にタブを追加する案も候補にあったが、popover 内の cursor 制御（`pointingHand` / `closedHand` / `CursorlessLabel`）との整合コストと、設定が増えたときの拡張性を考えて独立 NSWindow を採用。macOS 標準の System Settings 流の UX
- **Apply ボタンなし、即時保存**: 値変更時にその場で `PreferencesStore` に書き込み、`.kuraPreferencesDidChange` で各所が追従する。AppDelegate 側で定期再読み込みする必要がない
- **`launchAtLogin` を UserDefaults に保存しない**: 真の source of truth は `SMAppService` 側の登録状態。両方に保存すると「ユーザーがシステム設定で外した／別のアカウントで register した」ケースで不整合になるため、`PreferencesStore.launchAtLogin` の getter は毎回 `SMAppService.mainApp.status == .enabled` を見る。`register/unregister` が throw した場合は store の真値で UI を再同期（チェックが元に戻る）
- **起動時 fold (`foldOnLaunch`) の消化タイミング**: `applicationDidFinishLaunching` で `pendingFoldOnLaunch` に snapshot し（起動から warmup 成功までの間に環境設定で値を変えられても起動時の意図を尊重）、`applyScanResult` で **初回** `didCompleteAuthorizedScan = true` を立てるタイミングで `consumeFoldOnLaunchIfNeeded()` を呼ぶ。内部の `performToggleFold(silent: true)` 経由で detail scan 待ちや「セパレータが蔵の左にあるか」チェックなど折りたたみの安全機構を流用しつつ、失敗時の alert を抑止する（起動直後に modal を出さない）。セパレータが蔵の左にない場合も黙って諦め（次回起動でリトライ）
- **SF Symbol cache invalidation**: v0.6 では `static let` で起動時 1 度だけ build していたが、symbol 変更で再 build が必要なため AppDelegate の instance var (`expandedIcon` / `foldedIcon`) に変更。`rebuildIconCache()` を `applicationDidFinishLaunching` と `handlePreferencesDidChange` で呼ぶ。fold/expand 時 (高頻度) は cache を read するだけで build しなおさない
- **除外リストは scanner ではなく AppDelegate で適用**: `MenuBarLayoutScanner` には「事実 (位置で見つかったアプリ)」だけを返す責務を残し、policy (除外) は `AppDelegate.visibleApps` で適用する。scan 重コスト (AX 走査) と policy 変更 (UI 操作) は頻度も性質も違うため層を分ける。除外切り替えで scan は走らない
- **`lastScanResult` は除外前を保持**: 「対象アプリ」タブで除外解除 UI を出すために、除外済みアプリも `lastScanResult` に残す。VC への `setTargets` 直前の `visibleApps` で初めて filter が適用される
- **「対象アプリ」タブの行データは window snapshot**: 環境設定ウィンドウを開く度に AppDelegate から `setScanResult(lastScanResult)` で注入し、ウィンドウ表示中の scan 更新はリアルタイム反映しない。リアルタイム更新は table 行追加/削除アニメーションが絡んで複雑なため、開き直しで最新化する MVP 設計。除外済みだが scan に出ていない bundleId (起動していない or 蔵より右に移動した) は `buildDisplayedApps` で末尾に補完するため、起動していなくても除外解除できる
- **ホットキー録音は自前実装、外部依存を入れない**: 候補にあった MASShortcut は本機能のためだけに依存を増やすには大きすぎる。Kura で必要なのは「単一キー組み合わせの録音 + Carbon RegisterEventHotKey への橋渡し」だけなので、`HotKeyRecorderView` を ~240 行の `NSView` サブクラスで実装。`NSEvent` の `keyDown` + `charactersIgnoringModifiers` で十分カバーできる
- **ホットキー表示文字列は録音時に確定して保存**: keyCode + modifiers だけ保存して表示時に逆引きする案もあったが、特殊キー (kVK_F1, kVK_Return 等) と通常文字キーで逆引きロジックが分岐し、キーボードレイアウト変更にも追従しにくい。録音時の `event.charactersIgnoringModifiers` をそのまま `display` に焼き込めば、ユーザーが録音した瞬間に見た文字列がそのまま再表示される
- **modifier 1 つ以上を必須**: 単独キー (modifier なし) は OS や前面アプリのキーバインドと衝突しやすいため `HotKeyRecorderView.keyDown` で拒否しビープを鳴らす。F キー単独も同様に拒否される（実用上は modifier 必須で問題ない）
- **recording に入るのはマウスクリック経由のみ** (v0.8): Tab navigation で focus が当たった瞬間に recording モードに入る設計だと、次のキー入力 (Tab で別のフィールドに移ろうとした Space 等) で既存 hotkey が誤って上書きされてしまう。`mouseDown` で `pendingRecordingActivation` フラグを立てて `becomeFirstResponder` でそれを見ることで「クリック経由でだけ録音突入、それ以外は focus ring のみ」を表現する。Tab focus 中の Space/Return での録音開始は今のところサポートしない (将来の追加機能候補)
- **Escape は modifier なしのときだけ録音キャンセル** (v0.8): Cmd+Esc や Shift+Esc のような Escape を含む組み合わせも妥当な hotkey 候補なので、modifier 付きの Escape は通常の登録パスに流す。modifier なしの Escape 単独だけ「録音キャンセル」として扱う
- **録音中の `reloadFromStore` は recorder.hotKey を上書きしない** (v0.8): `.kuraPreferencesDidChange` や `showWindow` 経由で再同期が走ったとき、録音中の recorder の状態を尊重する (上書きすると入力途中の hotKey 候補が消える)。「デフォルトに戻す」ボタンだけは録音中でも反映したいので、`resetHotKey` で先に `makeFirstResponder(nil)` を呼んで明示的に録音を終了させてから reload する
- **HotKeyManager の install 失敗は `update` で retry 可能化** (v0.8): 旧設計だと `init` 内で `installSharedHandlerIfNeeded` が失敗した瞬間に early return していたため、`Self.handler` が未代入のまま固定化されていた。`update` で別キーが来ても handler が nil なので callback が silent no-op、ユーザーから見ると「設定したのに反応しない」状態。修正後は `init` で install を呼ばず `Self.handler` を必ず代入。`registerHotKey` の頭で idempotent な `installSharedHandlerIfNeeded` を呼び、install 失敗時は `current(KeyCode/Modifiers)` を更新せず return することで、次の `update` が同値で来ても差分判定をパスして再試行できる。`InstallEventHandler` は軽量なので毎回の試行コストも問題にならない
- **HotKeyManager の単一 instance 契約を assertion で明示** (v0.8): `Self.handler` を static slot に持つ設計上、複数 instance を作ると後勝ち上書きで誤発火源になる。`init` 冒頭で `assert(Self.handler == nil)` を入れ、テスト中や設計事故で 2 個目を作った瞬間に debug build で落とす。production では AppDelegate に 1 個のみ作る運用は変わらず、契約を型レベル外で明示するための assertion
- **`PreferencesStore.hotKey` の getter は `UInt32(exactly:)` で範囲外を弾く** (v0.8): UserDefaults には `Int` で保存しているため 64bit プラットフォームでは `UInt32` 範囲外の値も入りうる（手書き plist や別バージョン書き戻し）。`UInt32(_:)` での truncation を `RegisterEventHotKey` に渡すと未定義動作になるため、`UInt32(exactly:)` で範囲外なら `.default` に倒す。同じ guard 句で `keyCode > 0` も維持する（kVK_ANSI_A=0 とは別物、Carbon は 0 を未定義扱い）

### 含まれないもの

- v0.7 で予定していた 5 項目はすべて完了。v0.8 以降で追加するなら独立した節として追記する
...
- **v0.6** (完了): 蔵対象アプリの並び替え永続化（popover で D&D、`UserDefaults "kura.appOrder"` に bundleId 配列を保存）+ popover UI 刷新（`NSStackView` + 自前 `IconView` の 4 列アイコングリッド、クリックで `NSMenu` 展開、`.applicationDefined` 手動 close）+ 蔵アイコン SF Symbol 化（`archivebox` / `archivebox.fill` で fold 状態を視覚化）
- **v0.7** (完了): 環境設定 UI（独立 NSWindow、右クリックメニューから開く、`NSTabView` で「一般」「対象アプリ」「ホットキー」タブ）。蔵アイコン symbol 選択 / 起動時 fold 状態 / Mac 起動時自動起動（`SMAppService.mainApp`）/ 対象アプリ除外リスト (`AppExclusionStore` + NSTableView) / 折りたたみトグルのホットキーカスタマイズ (`HotKeyRecorderView` + `HotKeyManager.update`) の 5 設定をユーザー操作で変更可能化
- **v0.8** (完了): 品質固め — `/code-review max effort` で出た 12 件 findings を 3 PR (#20 録音 UX 修正 / #21 HotKeyManager 堅牢化 / quality-cleanup) に分けて全件消化
