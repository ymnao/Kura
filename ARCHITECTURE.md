...
- **`update(keyCode:modifiers:)`** で再登録経路を提供（v0.7 で追加）。登録中の `hotKeyRef` を instance プロパティに保持し、`update` 時に旧 ref を `UnregisterEventHotKey` してから新 ref を登録する。旧 ref の解除が失敗しても新規登録は試みる（Carbon は非排他登録なので残った旧キーの実害は限定的）。handler は init 時のものを継続使用する（Kura は toggleFold だけが対象）
...
`AppDelegate.handlePreferencesDidChange` は symbol / foldOnLaunch / appExclusion / hotKey すべての変更で同じ `.kuraPreferencesDidChange` を受け取るため、`lastRegisteredHotKey: KuraHotKey?` を持って `PreferencesStore.hotKey` と差分判定し、変化があった時だけ `hotKeyManager?.update(...)` を呼ぶ。無関係な設定変更で OS の Carbon 登録を毎回叩く無駄を避ける。
...
| `hotKey` (折りたたみトグル) | `UserDefaults "kura.hotKey.keyCode" / ".modifiers" / ".display"` (Int + Int + String) | 設定変更通知 → `handlePreferencesDidChange` で差分判定 → `HotKeyManager.update(keyCode:modifiers:)` |
