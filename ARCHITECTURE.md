...
├── PreferencesStore.swift            環境設定の永続化 (symbol / foldOnLaunch / launchAtLogin)。SMAppService 統合
├── PreferencesWindowController.swift 独立 NSWindow ベースの環境設定 UI (一般 / 対象アプリ タブ)
...
蔵アイコンのシンボル選択、起動時 fold 状態、Mac 起動時自動起動、対象アプリ除外リストの 4 設定をユーザーが UI から変更できる。

### 構成

- `PreferencesStore.swift`: 値ストア。`UserDefaults` と `SMAppService` を 1 箇所に集約。値変更時に `.kuraPreferencesDidChange` を post し、`AppDelegate` が購読してアイコン再描画する
...
### 含まれないもの（v0.7 後続 PR で対応予定）

- ホットキーカスタマイズ（MASShortcut 依存 or 自前実装、別 PR）
...
- **v0.7** (進行中): 環境設定 UI（独立 NSWindow、右クリックメニューから開く、`NSTabView` で「一般」「対象アプリ」タブ）。蔵アイコン symbol 選択 / 起動時 fold 状態 / Mac 起動時自動起動（`SMAppService.mainApp`）を foundation として実装、続けて対象アプリ除外リスト (`AppExclusionStore` + NSTableView) を追加。残るホットキーカスタマイズは後続 PR 予定
