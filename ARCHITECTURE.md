...
├── PreferencesWindowController.swift 独立 NSWindow ベースの環境設定 UI
...
蔵アイコンのシンボル選択、起動時 fold 状態、Mac 起動時自動起動の 3 設定をユーザーが UI から変更できる。
...
- `PreferencesWindowController.swift`: 独立 NSWindow ベースの UI（`NSStackView` レイアウト）。`isReleasedWhenClosed = false` でインスタンスを保持し、AppDelegate が 1 個だけ生成して以降は再 show のみ
- 入り口: 蔵の右クリックメニュー「環境設定…（⌘,）」。`AppDelegate.showPreferences(_:)` から `controller.showWindow(nil)` + `NSApp.activate(ignoringOtherApps: true)`
...
- 対象アプリ除外リスト（NSTableView 必要、別 PR）
...
- **v0.7** (進行中): 環境設定 UI（独立 NSWindow、右クリックメニューから ⌘,）。蔵アイコン symbol 選択 / 起動時 fold 状態 / Mac 起動時自動起動（`SMAppService.mainApp`）を foundation として実装。後続 PR で対象アプリ除外リストとホットキーカスタマイズを追加予定
