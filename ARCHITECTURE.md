...
- `HotKeyRecorderView.swift`: System Settings 流のキー録音 UI。`NSView` サブクラスで `becomeFirstResponder` 時に recording モードへ、次の `keyDown` を capture して `KuraHotKey` を組み立てる。`NSEvent.ModifierFlags` → Carbon `cmdKey | optionKey | controlKey | shiftKey` への変換と、`kVK_*` ベースの特殊キー名 (↩ / ⇥ / Space / 矢印 / F1-F12 等) の表示文字列組み立てを担う
...
- **ホットキー録音は自前実装、外部依存を入れない**: 候補にあった MASShortcut は本機能のためだけに依存を増やすには大きすぎる。Kura で必要なのは「単一キー組み合わせの録音 + Carbon RegisterEventHotKey への橋渡し」だけなので、`HotKeyRecorderView` を ~150 行の `NSView` サブクラスで実装。`NSEvent` の `keyDown` + `charactersIgnoringModifiers` で十分カバーできる
