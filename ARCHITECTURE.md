...
- **起動時 fold (`foldOnLaunch`) の消化タイミング**: `applyScanResult` で **初回** `didCompleteAuthorizedScan = true` を立てるタイミングで `consumePendingFoldOnLaunchIfNeeded()` を呼ぶ。内部では既存の `toggleFold(nil)` を経由するため、detail scan 待ちや「セパレータが蔵の左にあるか」チェックなど折りたたみの安全機構をそのまま流用できる。セパレータが蔵の左にない場合は黙って諦め（次回起動でリトライ、alert は出さない）
